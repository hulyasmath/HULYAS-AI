...
class PDFManager {
  ...
  async initialize() {
    ...
    try {
      // DISABLED: PDF.js loading blocked by CSP in Chrome extensions
      // PDF functionality disabled to prevent CSP violations
      console.warn('⚠️ PDF Manager: Disabled due to Content Security Policy restrictions');
      ...
      // These PDFs should be in chrome-extension/docs/ folder
      const bundledPDFs = [
      ...
      const loadPromises = bundledPDFs.map(pdfName => {
        try {
          // Get PDF URL - works in Chrome extension context
          let pdfUrl;
          if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.getURL) {
            pdfUrl = chrome.runtime.getURL(`docs/${pdfName}`);
          } else if (typeof browser !== 'undefined' && browser.runtime && browser.runtime.getURL) {
            pdfUrl = browser.runtime.getURL(`docs/${pdfName}`);
          } else {
            // Fallback for non-extension context (standalone HTML)
            pdfUrl = `docs/${pdfName}`;
          }
