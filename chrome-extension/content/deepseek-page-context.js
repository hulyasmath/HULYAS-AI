// Zeq OS Network Interception - Page Context Script
    ...
    if (typeof url === 'string' && options.body) {
      ...
      if (
        ...
        try {
          ...
          // EXACTLY like index.html line 16928: Find the last user message
          if (bodyData.messages && Array.isArray(bodyData.messages)) {
            ...
            if (lastUserMessage) {
              ...
              // Process ALL messages - don't skip if they start with '{' because our prompt starts with text summary
              if (originalContent && originalContent.length > 0) {
                // Only skip if it's already our mathematical prompt (contains frameworkDescription)
                if (!originalContent.includes('frameworkDescription') && !originalContent.includes('MAIN 42 OPERATORS')) {
                  needsProcessing = true;
                  console.log('📝 Zeq OS [Page Context]: Found user message to process:', originalContent.substring(0, 50));
                } else {
                  console.log('⏭️ Zeq OS [Page Context]: Message already processed (contains framework info)');
                }
              }
            }
          ...
          if (needsProcessing) {
            ...
            return new Promise((resolve, reject) => {
              ...
              const timeout = setTimeout(() => {
                if (!responded) {
                  responded = true;
                  console.warn('⏱️ Zeq OS [Page Context]: Processing timeout (2s), using original message');
              ...
              const handler = (event) => {
                ...
                if (event.data && event.data.type === 'ZEQ_PROCESSED_RESPONSE' && event.data.requestId === requestId) {
                  ...
                  console.log('✅ Zeq OS [Page Context]: Received processed message, length:', processedContent.length);
                  console.log('📋 Zeq OS [Page Context]: Processed content preview (first 500 chars):', processedContent.substring(0, 500));
                  
                  // Verify it contains framework information
                  const hasFramework = processedContent.includes('frameworkDescription');
                  const hasMain42 = processedContent.includes('MAIN 42 OPERATORS') || processedContent.includes('main42Operators');
                  const hasON0 = processedContent.includes('ON0');
                  console.log('✅ Zeq OS [Page Context]: Content verification:', {
                    hasFramework,
                    hasMain42,
                    hasON0,
                  ...
                  if (!hasFramework || !hasMain42) {
                    console.error('❌ Zeq OS [Page Context]: CRITICAL - Processed content missing framework information!');
                  }
                  ...
                  if (bodyData.messages) {
                    ...
                    for (let i = bodyData.messages.length - 1; i >= 0; i--) {
                      if (bodyData.messages[i].role === 'user' || bodyData.messages[i].role === 'User') {
                        ...
                        bodyData.messages[i].content = processedContent;
                        bodyData.messages[i].text = processedContent;
                        console.log('✅ Zeq OS [Page Context]: Replaced user message at index', i, 'with mathematical prompt');
                        console.log('📤 Zeq OS [Page Context]: Final message content preview:', bodyData.messages[i].content.substring(0, 300));
                  ...
                  console.log('🚀 Zeq OS [Page Context]: Sending processed message to DeepSeek API');
                  resolve(originalFetch.apply(this, args));
                }
              };
