...
const maybeUninstallOAuthMCP = async (userId, pluginKey, appConfig) => {
  ...
  const oauthServers = await MCPServersRegistry.getInstance().getOAuthServers();
...
/**
 * PUT /api/user/plan
 * Update user's own plan
 */
const updateUserPlanController = async (req, res) => {
  try {
    logger.info(`[updateUserPlanController] Request received:`, {
      method: req.method,
      path: req.path,
      url: req.url,
      body: req.body,
    ...
    if (!req.user || !req.user.id) {
      logger.error('[updateUserPlanController] No user in request');
      return res.status(401).json({ message: 'Unauthorized - user not authenticated' });
    }
    
    const userId = req.user.id;

    logger.info(`[updateUserPlanController] Request to update user ${userId} to plan ${planId}`);

    if (!planId) {
      logger.warn('[updateUserPlanController] Plan ID missing');
    ...
    if (!plan) {
      logger.warn(`[updateUserPlanController] Plan ${planId} not found`);
      return res.status(400).json({ message: 'Invalid plan ID' });
    }

    logger.info(`[updateUserPlanController] Found plan: ${plan.name} (${plan.displayName})`);

    // If plan has stripePriceId, user must use Stripe checkout
    // Only allow direct plan switching for free plan or plans without Stripe
    if (plan.stripePriceId && plan.name !== 'free') {
      logger.warn(`[updateUserPlanController] Plan ${plan.name} requires Stripe checkout`);
      return res.status(400).json({ 
        message: 'This plan requires payment. Please use the upgrade button to proceed with checkout.' 
      });
    }

    // Get current user before update
    const userBefore = await User.findById(userId).populate('planId').lean();
    logger.info(`[updateUserPlanController] User current plan: ${userBefore.planId?.name || 'none'}`);

    // Update user's plan
    await updateUser(userId, { planId: planId });
    logger.info(`[updateUserPlanController] Updated user ${userId} planId to ${planId}`);
    
    // Fetch updated user with populated plan
    const user = await User.findById(userId).populate('planId').lean();
    logger.info(`[updateUserPlanController] User new plan: ${user.planId?.name || 'none'}`);

    // Return full user object with populated plan for frontend
    const response = {
    ...
    logger.info(`[updateUserPlanController] Sending response:`, JSON.stringify(response));
    res.status(200).json(response);
  } catch (error) {
    logger.error('[updateUserPlanController] Error:', error);
