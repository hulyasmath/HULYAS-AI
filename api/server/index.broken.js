...
const { generateDailyPatterns } = require('./services/ZeqPatternGenerator');
...
const startServer = async () => {
  ...
  // CORS configuration for App Store integration
  const corsOptions = {
    origin: [
      process.env.DOMAIN_CLIENT || 'http://localhost:3080',
      'http://localhost:3001',  // Zeq OS App Store
  ...
  app.use('/api/admin/llm-config', routes.llmConfig);
  ...
  app.use('/api/hulyas', routes.hulyasChat);
  ...
  app.listen(port, host, async () => {
    ...
    // Daily pattern generation scheduler
    // Runs every 24 hours (86400000ms) starting from now
    const DAILY_PATTERN_COUNT = 3;
    const runDailyPatternGeneration = async () => {
      try {
        const generated = await generateDailyPatterns(DAILY_PATTERN_COUNT);
        logger.info(`[DailyPatternGenerator] Generated ${generated.length} new patterns`);
      } catch (error) {
        logger.error('[DailyPatternGenerator] Error:', error.message);
      }
    };

    // Schedule daily generation (every 24 hours)
    setInterval(runDailyPatternGeneration, 24 * 60 * 60 * 1000);
    logger.info('[DailyPatternGenerator] Scheduled to run every 24 hours');
  });
};
