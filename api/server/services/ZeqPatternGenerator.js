...
async function generateDailyPatterns(count) {
  ...
  try {
    // Check if today already has patterns
...
function initDailyScheduler() {
  ...
  setInterval(async () => {
    try {
      ...
      logger.info(`[ZeqPatternGenerator] Daily generation: ${patterns.length} patterns selected`);
    } catch (error) {
      logger.error('[ZeqPatternGenerator] Daily generation error:', error.message);
    }
  }, TWENTY_FOUR_HOURS);

  logger.info('[ZeqPatternGenerator] Daily scheduler initialized (every 24 hours)');
}
