...
const registerUser = async (user, additionalData = {}) => {
  ...
  if (error) {
    ...
    const { email, password, name, username, provider, planId } = user;
  ...
  try {
    ...
    // Assign plan to new user (from registration or default to free)
    try {
      let planToAssign = null;
      if (planId) {
        // User selected a plan during registration
        planToAssign = await getPlanById(planId);
        if (!planToAssign) {
          logger.warn(`[registerUser] Selected plan ${planId} not found, defaulting to free`);
        }
      }
      
      // If no plan selected or plan not found, default to free
      if (!planToAssign) {
        planToAssign = await getPlanByName('free');
      }
      
      if (planToAssign) {
        await updateUser(newUserId, { planId: planToAssign._id });
        logger.info(`[registerUser] Assigned ${planToAssign.name} plan to new user [Email: ${email}]`);
      } else {
        logger.warn('[registerUser] Free plan not found, user created without plan');
      }
    } catch (planError) {
      logger.error('[registerUser] Error assigning plan to new user:', planError);
