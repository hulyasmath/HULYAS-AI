...
const formatAgentMessages = (payload) => {
  ...
  for (const message of payload) {
    ...
    // Filter out unsupported content types (like 'file') before formatting
    // This prevents "unknown variant `file`, expected `text`" errors
    if (Array.isArray(message.content)) {
      const originalLength = message.content.length;
      message.content = message.content.filter((part) => {
        if (!part || typeof part !== 'object' || !part.type) {
          return false; // Skip invalid parts
        }
        
        // Explicitly reject 'file' type (not supported by LangChain)
        if (part.type === 'file') {
          return false;
        }
        ...
        return (
          ...
          part.type === ContentTypes.IMAGE_URL ||
          part.type === ContentTypes.TOOL_CALL ||
          part.type === ContentTypes.THINK ||
          part.type === ContentTypes.ERROR ||
          part.type === ContentTypes.AGENT_UPDATE ||
          (part.type === ContentTypes.IMAGE_FILE && message.role === 'user') // Allow IMAGE_FILE for user messages (vision)
        );
      });
      
      // If all content was filtered out and message had content, add a text placeholder
      if (message.content.length === 0 && originalLength > 0) {
        message.content = [{ type: ContentTypes.TEXT, [ContentTypes.TEXT]: '' }];
      }
    }
    ...
    for (const part of message.content) {
      if (part.type === ContentTypes.TEXT && part.tool_call_ids) {
        ...
        continue;
      } else if (part.type === 'file' || part.type === ContentTypes.IMAGE_FILE) {
