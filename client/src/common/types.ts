...
declare global {
  interface Window {
    ...
    ZeqOSFramework?: {
      ...
      UTPWithOperators?: new (mode: string, frequency: number) => UTPWithOperators;
      version?: string;
      frameworkName?: string;
    };
    zeqMiddleware?: ZeqOSMiddleware;
    utpFramework?: UTPWithOperators;
    zeqMathematicalPrompt?: string | null;
    utpOperatorData?: unknown;
    processMathematicalFramework?: (message: string, conversationId?: string | null) => Promise<{
      zeqResult: {
...
interface TransparencyManager {
  ...
  saveToServer(conversationId: string, logEntry: unknown): Promise<unknown>;
  getLog(): TransparencyLogEntry[];
  ...
  getHistoricalPatterns(): Promise<unknown>;
  compareToHistory(currentState: unknown): Promise<unknown>;
  cachePatterns(patterns: unknown): void;
  getOperatorSuccessRate(operator: string): number;
}
...
interface TransparencyTranscript {
  ...
  completeDataStream: unknown[];
  allReasoningSteps: unknown[];
  systemInfo: {
    [key: string]: unknown;
  };
}
