...
export default function useAppStartup({
  ...
  /** Set the app title */
  useEffect(() => {
    const appTitle = startupConfig?.appTitle ?? 'Zeq OS 1.287 Hz';
    // Set default title, but don't override if conversation title is already set
    if (!document.title || document.title === 'Zeq OS 1.287 Hz' || !document.title.includes('|')) {
