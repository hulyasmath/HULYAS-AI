...
// Removed: processThroughFramework - framework processing happens in useSubmitMessage
...
export default function useChatFunctions({
  ...
  // Zeq OS Mathematical Framework - Process framework but KEEP original text for display
  // Framework processing happens in useSubmitMessage, injection happens in useSSE
  // User should only see their original message, NOT the processed prompt
  // Ensure original text is preserved - never use the mathematical prompt in the UI
  const ask: TAskFunction = (
    ...
    // Zeq OS Mathematical Framework - DO NOT process here
    // Framework processing happens in useSubmitMessage (before ask() is called)
    // The processed prompt is injected at API level in useSSE.ts
    // User should only see their original message, NOT the processed prompt
    console.log('📤 Zeq OS: ask() called with original text (user sees this):', text.substring(0, 100));
    // DO NOT replace text here - keep original for display
    // Framework will be injected at API level via window.zeqMathematicalPrompt
    ...
    // CRITICAL: Always preserve the original user text, never use the processed mathematical prompt
    // The mathematical prompt is sent to LLM via network interceptor, but UI always shows original
    const originalText = text; // This is the original user message from useSubmitMessage
    
    // Store original text for later retrieval if mathematical prompt somehow gets into message
    if (typeof window !== 'undefined' && originalText) {
      const msgId = isContinued && messageId != null && messageId ? messageId : intermediateId;
      window.__zeqOriginalMessages = window.__zeqOriginalMessages || {};
      window.__zeqOriginalMessages[msgId] = originalText;
    }
    
    const currentMsg: TMessage = {
      text: originalText, // Always use original text for display
      sender: 'User',
