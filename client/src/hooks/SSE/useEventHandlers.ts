...
export default function useEventHandlers({
  ...
  const finalHandler = useCallback(
    (data: TFinalResData, submission: EventSubmission) => {
      ...
      // Log AI response to transparency manager
      if (
        typeof window !== 'undefined' &&
        window.transparencyManager &&
        conversation.conversationId &&
        responseMessage
      ) {
        const aiResponseText = getAllContentText(responseMessage);
        window.transparencyManager
          .logAIResponse({
          ...
          .catch((err) => {
            console.warn('⚠️ Zeq OS: AI response transparency logging failed', err);
          });
      }
