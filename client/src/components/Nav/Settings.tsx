...
import { MessageSquare, Command, DollarSign, Key, Crown, Settings2, CreditCard } from 'lucide-react';
...
export default function Settings({ open, onOpenChange }: TDialogProps) {
  ...
  // Show MCP API Key tab to all authenticated users - backend will validate paid plan
  const showMCPApiKey = !!user;
  ...
  const handleKeyDown = (event: React.KeyboardEvent) => {
    const tabs: SettingsTabValues[] = [
      ...
      ...(showMCPApiKey ? [SettingsTabValues.MCP_API_KEY] : []),
      SettingsTabValues.SUBSCRIPTION,
      ...(isAdmin ? [SettingsTabValues.ADMIN_SETTINGS, SettingsTabValues.STRIPE_SETTINGS, SettingsTabValues.PLANS] : []),
    ];
  ...
  const settingsTabs: {
      ...
      : ([] as { value: SettingsTabValues; icon: React.JSX.Element; label: TranslationKeys }[])),
    {
      ...
      ? [
          {
            value: SettingsTabValues.MCP_API_KEY,
            icon: <Key className="icon-sm" />,
            label: 'com_nav_setting_mcp_api_key' as TranslationKeys,
          },
        ]
      : []),
      {
        value: SettingsTabValues.SUBSCRIPTION,
        icon: <Crown className="icon-sm" />,
        label: 'com_nav_setting_subscription' as TranslationKeys,
      },
      ...(isAdmin
      ? [
          {
            value: SettingsTabValues.ADMIN_SETTINGS,
            icon: <Settings2 className="icon-sm" />,
            label: 'com_nav_setting_admin' as TranslationKeys,
          },
          {
            value: SettingsTabValues.STRIPE_SETTINGS,
            icon: <CreditCard className="icon-sm" />,
            label: 'com_nav_setting_stripe' as TranslationKeys,
          },
          {
            value: SettingsTabValues.PLANS,
            icon: <Crown className="icon-sm" />,
            label: 'com_nav_setting_plans' as TranslationKeys,
          },
        ]
      : []),
  ];
              ...
              <div className="max-h-[550px] overflow-auto px-6 md:max-h-[400px] md:min-h-[400px] md:w-[680px]">
                ...
                >
                  ...
                  <div className="overflow-auto sm:w-full sm:max-w-none md:pr-0.5 md:pt-0.5">
                    ...
                    {showMCPApiKey && (
                      <Tabs.Content value={SettingsTabValues.MCP_API_KEY} tabIndex={-1}>
                        <MCPApiKey />
                      </Tabs.Content>
                    )}
                    ...
                    {isAdmin && (
                      <>
                        <Tabs.Content value={SettingsTabValues.ADMIN_SETTINGS} tabIndex={-1}>
                          <AdminSettings />
                        </Tabs.Content>
                        <Tabs.Content value={SettingsTabValues.STRIPE_SETTINGS} tabIndex={-1}>
                          <StripeSettings />
                        </Tabs.Content>
                        <Tabs.Content value={SettingsTabValues.PLANS} tabIndex={-1}>
                          <Plans />
                        </Tabs.Content>
                      </>
                    )}
