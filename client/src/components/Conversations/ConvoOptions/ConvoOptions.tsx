...
import { Ellipsis, Share2, Copy, Archive, Pen, Trash, FileJson } from 'lucide-react';
...
import TransparencyDownloadButton from './TransparencyDownloadButton';
...
function ConvoOptions({
  ...
  const transparencyButtonRef = useRef<HTMLButtonElement>(null);
  ...
  const dropdownItems = useMemo(
    () => [
      ...
      {
        label: 'Download Transparency',
        onClick: () => {
          // Handle click in TransparencyDownloadButton component
        },
        icon: <FileJson className="icon-sm mr-2 text-text-primary" />,
        show: true,
        hideOnClick: true,
        ref: transparencyButtonRef,
        render: (props) => (
          <TransparencyDownloadButton
            {...props}
            conversationId={conversationId}
            onDownloadComplete={() => setIsPopoverActive(false)}
          />
        ),
      },
      {
