...
export const updateUserPlan = (payload: { planId: string }): Promise<{ message: string; user: any }> => {
  return request.put(endpoints.updateUserPlan(), payload);
};
...
export interface TCreatePlanRequest {
  name: string;
  displayName: string;
  monthlyTokenLimit?: number | null;
  dailyRequestLimit?: number | null;
...
export interface TUpdatePlanRequest {
  displayName?: string;
  monthlyTokenLimit?: number | null;
  dailyRequestLimit?: number | null;
  allowedEndpoints?: string[];
...
export interface TUserUsage {
  monthlyTokens: number;
  dailyRequests: number;
}

export interface TUserPlanInfo {
  plan: TPlan | null;
  usage: TUserUsage;
  customTokenOverrides: number;
  stripeCustomerId: string | null;
  stripeSubscriptionId: string | null;
}

export interface TUpdateUserPlanRequest {
  planId?: string | null;
  customTokenOverrides?: number | null;
  stripeCustomerId?: string | null;
  stripeSubscriptionId?: string | null;
}

export function getAdminPlans(): Promise<TPlan[]> {
  return request.get(endpoints.adminPlans());
}

export function getPublicPlans(): Promise<TPlan[]> {
  return request.get(endpoints.publicPlans());
}

// Stripe functions
export function createStripeCheckoutSession(payload: { planId: string }): Promise<{ sessionId: string; url: string }> {
  return request.post(endpoints.stripeCreateCheckoutSession(), payload);
}

export function createStripeCustomerPortalSession(): Promise<{ url: string }> {
  return request.post(endpoints.stripeCustomerPortal());
}

// Admin Settings functions
export function getAdminApiConfig(): Promise<any> {
  return request.get(endpoints.adminSettingsEndpoints.apiConfig());
}

export function updateAdminApiConfig(payload: { service: string; envVar: string; value: string }): Promise<{ message: string; service: string; envVar: string; note: string }> {
  return request.put(endpoints.adminSettingsEndpoints.updateApiConfig(), payload);
}

export function getAdminUserKeys(userId?: string): Promise<any> {
  const url = userId 
    ? `${endpoints.adminSettingsEndpoints.userKeys()}?userId=${userId}`
    : endpoints.adminSettingsEndpoints.userKeys();
  return request.get(url);
}

export function deleteAdminUserKey(userId: string, keyName: string): Promise<{ message: string }> {
  return request.delete(endpoints.adminSettingsEndpoints.deleteUserKey(userId, keyName));
}

export function getAdminApiKeys(): Promise<any[]> {
  return request.get(endpoints.adminSettingsEndpoints.apiKeys());
}

export function updateAdminApiKey(payload: { endpointName: string; apiKey: string }): Promise<{ message: string; endpointName: string; envVar: string; note: string }> {
...
export function createAdminPlan(data: TCreatePlanRequest): Promise<TPlan> {
  return request.post(endpoints.adminPlans(), data);
}

export function updateAdminPlan(planId: string, data: TUpdatePlanRequest): Promise<TPlan> {
  return request.put(endpoints.adminPlan(planId), data);
}

export function deleteAdminPlan(planId: string): Promise<{ message: string; plan: TPlan }> {
  return request.delete(endpoints.adminPlan(planId));
}

export function getAdminUserUsage(userId: string): Promise<TUserUsage> {
  return request.get(endpoints.adminUserUsage(userId));
}

export function getAdminUserPlan(userId: string): Promise<TUserPlanInfo> {
  return request.get(endpoints.adminUserPlan(userId));
}
