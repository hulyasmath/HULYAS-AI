...
async function _put(url: string, data?: any) {
  ...
  // Check if we got HTML instead of JSON (indicates routing error)
  if (typeof response.data === 'string' && response.data.trim().startsWith('<!DOCTYPE')) {
    throw new Error(`Server returned HTML instead of JSON. This usually means the route ${url} was not found. Status: ${response.status}`);
  }
